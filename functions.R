# ddB = function(x,W.=W,C.=C,sigT.=sigT){
#   p = length(W.)
...
#   q = length(C.)
#   N = nrow(X.)
#   Sigma=sseXY(W., C., P_Yosc., P_Xosc., x,sigX.,sigY.,sigH.,sigT.,sigTo.,sigUo.)
...
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sqrt(x),sigY.,sigH.,sigT.,sigTo.,sigUo.))
#   dS = blockm(diag(1,p),diag(0,p,q),diag(0,q))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# dLdsig2Y = function(y,X.=X,Y.=Y,W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo,B_T.=B_T,
#                     sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo){
#   p = length(W.)
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sigX.,sqrt(y),sigH.,sigT.,sigTo.,sigUo.))
#   dS = blockm(diag(0,p),diag(0,p,q),diag(1,q))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# dLdsig2H = function(x,X.=X,Y.=Y,W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo,B_T.=B_T,
#                     sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo){
#   p = length(W.)
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sigX.,sigY.,sqrt(x),sigT.,sigTo.,sigUo.))
#   dS = blockm(diag(0,p),diag(0,p,q),tcrossprod(C.))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# dLdsig2T = function(x,X.=X,Y.=Y,W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo,B_T.=B_T,
#                     sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo){
#   p = length(W.)
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sigX.,sigY.,sigH.,sqrt(x),sigTo.,sigUo.))
#   dS = blockm(tcrossprod(W.),c(B_T.)*W.%*%t(C.),c(B_T.)^2*tcrossprod(C.))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# dLdsig2To = function(x,X.=X,Y.=Y,W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo,B_T.=B_T,
#                      sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo){
#   p = length(W.)
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sigX.,sigY.,sigH.,sigT.,sqrt(x),sigUo.))
#   dS = blockm(tcrossprod(P_Yosc.),diag(0,p,q),diag(0,q))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# dLdsig2Uo = function(x,X.=X,Y.=Y,W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo,B_T.=B_T,
#                      sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo){
#   p = length(W.)
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sigX.,sigY.,sigH.,sigT.,sigTo.,sqrt(x)))
#   dS = blockm(diag(0,p),diag(0,p,q),tcrossprod(P_Xosc.))
#   return(tr(0.5*crossprod(cbind(X.,Y.))%*%invS%*%dS%*%invS - N/2*invS%*%dS))
# }
# ddsig2 = function(sig2,...)
# {
#   #sig2 = exp(sig2)
#   x1 = dLdsig2X(sig2[1],sigY.=sqrt(sig2[2]),sigH.=sqrt(sig2[3]),sigT.=sqrt(sig2[4]),...)
#   x2 = dLdsig2Y(sig2[2],sigX.=sqrt(sig2[1]),sigH.=sqrt(sig2[3]),sigT.=sqrt(sig2[4]),...)
#   x3 = dLdsig2H(sig2[3],sigT.=sqrt(sig2[4]),sigX.=sqrt(sig2[1]),sigY.=sqrt(sig2[2]),...)
#   x4 = dLdsig2T(sig2[4],sigH.=sqrt(sig2[3]),sigX.=sqrt(sig2[1]),sigY.=sqrt(sig2[2]),...)
...
#   q = length(C.)
#   N = nrow(X.)
#   invS = solve(sseXY(W.,C.,P_Yosc.,P_Xosc.,B_T.,sig2[1]^0.5,sig2[2]^0.5,sig2[3]^0.5,sig2[4]^0.5,sig2[5]^0.5,sig2[6]^0.5))
#   dS = list(blockm(diag(1,p),diag(0,p,q),diag(0,q)),
#             blockm(diag(0,p),diag(0,p,q),diag(1,q)),
#             blockm(diag(0,p),diag(0,p,q),tcrossprod(C.)),
#             blockm(tcrossprod(W.),c(B_T.)*W.%*%t(C.),c(B_T.)^2*tcrossprod(C.)),
...
sseXY <- function(W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo, B_T.=B_T,
  ...
  if(length(sigX.)==6){
    sig=sigX.
    sigX.=sig[1];sigY.=sig[2];sigH.=sig[3];sigT.=sig[4];sigTo.=sig[5];sigUo.=sig[6]
  }
  if(length(sigX.)==4){
    sig=sigX.
    sigX.=sig[1];sigY.=sig[2];sigH.=sig[3];sigT.=sig[4];sigTo.=0;sigUo.=0
  }
  p = length(W.)
  q = length(C.)
  B_T.=matrix(B_T.)
...
logl <- function(W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo, B_T.=B_T, Dat=cbind(X,Y),
                 sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo)
...
logl_M <- function(W.=W, C.=C, P_Yosc.=PYo, P_Xosc.=PXo, B_T.=B_T, Dat=cbind(X,Y),
                 sigX.=sigX,sigY.=sigY,sigH.=sigH,sigT.=sigT,sigTo.=sigTo,sigUo.=sigUo)
  #input: Loadings, parameters
  #output: log-likelihood of Probabilistic O2PLS
{
  ...
  # Sigma X
  SX. = tcrossprod(W.%*%sigT.)+sigX.^2*diag(1,nrow=p)
  SY. = tcrossprod(C.%*%B_T.%*%sigT.) + tcrossprod(C.%*%sigH.) + sigY.^2*diag(1,nrow=q)
  SXY. = W.%*%sigT.^2%*%B_T.%*%t(C.)
  ...
  #for(i in 1:N){som = som + t(Data[i,])%*%invSobs.%*%t(t(Data[i,]))}
  #som=sum(sapply(1:N,function(i){t(Data[i,])%*%invSobs.%*%t(t(Data[i,]))}))
  som = sum(diag(crossprod(cbind(X.,Y.))%*%invSfull))
  logdet = c(determinant(Sfull)$mod)
  Q = - N*0.5*logdet - 0.5*som
  return(Q)
}
...
summ.po2m <- function(fit,X,Y){
  if(length(fit$sig)==4){fit$sig[5:6]=0}
  sigs = c(E=fit$sig[1]^2*p/ssq(X)*N,F=fit$sig[2]^2*q/ssq(Y)*N,T=fit$sig[4]^2/ssq(X)*N,U=(fit$sig[4]^2*fit$B^2+fit$sig[3]^2)/ssq(Y)*N,To=fit$sig[5]^2/ssq(X)*N,Uo=fit$sig[6]^2/ssq(Y)*N,H=fit$sig[3]^2/(fit$sig[4]^2*fit$B^2+fit$sig[3]^2))
  a=length((fit$W)[1])
  Mname=list(c("Comp","E","F","T",'U','To',"Uo","H"),c(""))
...
#   tic=proc.time()
#   cl_flag=F
#   if(is.null(cl) && parall){cl_flag=T;cl <- makeCluster(rep( 'localhost', detectCores()),type='SOCK')}
#   clusterExport(cl=cl, varlist=c("X_cv","Y_cv"),envir=environment())
#   invisible(clusterEvalQ(cl, source('C:/Users/selbouhaddani/OneDrive/LUMC/PhD/Rcode/Functions/functions.R')))
#   invisible(clusterEvalQ(cl, source('C:/Users/selbouhaddani/OneDrive/LUMC/PhD/Rcode/Functions/functions_stripped.R')))
#   #invisible(clusterCall(cl, function() library(o2plsCpp)))
#   for(a_i in seta){
...
#     }
#   }
#   on.exit(if(cl_flag){stopCluster(cl)})
#   print(proc.time()-tic)
#   return(best_par)
# }
# 
