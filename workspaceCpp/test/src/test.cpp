...
// Output: Loglikelihood of the data given parameters
  ...
  MatrixXd temp = Dat.transpose()*Dat*invS; // .trace() does not work directly, i.e. (...).trace()
  double som2 = temp.trace(); //Can do it faster, i think
  ...
  Logdiag = L.diagonal();
  double Logdiag2;
  for(unsigned int j=0;j<(p+q);j++){
	  Logdiag2 = Logdiag(j);
	  Logdiag(j) = 2 * log( Logdiag2 );   // Elementwise log (*2 since L is square root of NewS)
  }
...
VectorXd findrC(VectorXd lambda, VectorXd Cxt, VectorXd Cxto, double Ctt, double Ctto, double Ctoto)
...
// This function is used to solve findr1(lambda2) == 0 for lambda2
  ...
  double alp2 = Ctoto + lambda(1);
  double alp1 = Ctt + lambda(0);

  // Vectors P and W
  VectorXd Pl = (Cxto*alp1 - Cxt*Ctto) / (alp1*alp2 - Ctto*Ctto);
  VectorXd Wl = (Cxt - Pl*Ctto) / alp1;

  double tst1 = Wl.squaredNorm() - 1;
  double tst2 = Pl.squaredNorm() - 1;
...
MatrixXd findrC_matrix(VectorXd lambda, VectorXd Cxt, VectorXd Cxto, double Ctt, double Ctto, double Ctoto)
  {// Input: Lagrange multiplier lambda2, sufficient statistics C..
// Output: The norm of W minus 1
// This function is used to solve findr1(lambda2) == 0 for lambda2
...
MatrixXd jacrC(VectorXd lambda, VectorXd Cxt, VectorXd Cxto, double Ctt, double Ctto, double Ctoto)
{
  double alp2 = Ctoto + lambda(1);
  double alp1 = Ctt + lambda(0);
  ...
  VectorXd Pteller = (Cxto*alp1 - Cxt*Ctto);
  double Pnoemer = (alp1*alp2 - pow(Ctto,2));

  VectorXd P1 = (Pnoemer * Cxto - Pteller * alp2) / pow(Pnoemer,2);
  VectorXd P2 = -Pteller * alp1 / pow(Pnoemer,2);
  double W1 = Wl.transpose() * ((-alp1 * Ctto*P1 - Cxt + Pl*Ctto) / pow(alp1,2));
  double W2 = Wl.transpose() * (-Ctto * P2 / pow(alp1,2));
  double P11 = Pl.transpose() * P1;
  double P12 = Pl.transpose() * P2;
...
VectorXd NewtonStepC(VectorXd x,VectorXd Cxt, VectorXd Cxto, double Ctt, double Ctto, double Ctoto){
  MatrixXd Jac = (jacrC(x,Cxt,Cxto,Ctt,Ctto,Ctoto));
  MatrixXd Jinv(2,2);
  Jinv << Jac(1,1), -Jac(0,1),
          -Jac(1,0), Jac(0,0);
  Jinv *= 1/(Jac(0,0)*Jac(1,1) - Jac(1,0)*Jac(0,1));
  VectorXd fx = (findrC(x,Cxt,Cxto,Ctt,Ctto,Ctoto));
  return x - Jinv * fx;
}

VectorXd multirootC(VectorXd start, VectorXd Cxt, VectorXd Cxto, double Ctt, double Ctto, double Ctoto,int maxit=1000000,double atol=0.000001){

  VectorXd xnew = start;
  VectorXd resid = (findrC(xnew,Cxt,Cxto,Ctt,Ctto,Ctoto));
  int iternr = 0;

  for(int i=1;i<maxit;i++){
    resid = (findrC(xnew,Cxt,Cxto,Ctt,Ctto,Ctoto));
    if(resid.squaredNorm() < atol*atol){break;}
    xnew = NewtonStepC(xnew,Cxt,Cxto,Ctt,Ctto,Ctoto);
    iternr++;
  }
  ...
  List ret;
  ret["root"] = xnew;
  ret["f.root"] = Rcpp::as<Eigen::VectorXd>(f(xnew,Cxt,Cxto,Ctt,Ctto,Ctoto));
  ret["iterations"] = iternr;
  ret["accuracy"] = resid.norm();
  ret["status"] = type;
  */
  return ret;
}
...
List EMstepC(VectorXd W,VectorXd C,VectorXd P_Yosc,VectorXd P_Xosc,double B_T, MatrixXd Dat,
              double sig2X,double sig2Y,double sig2H,double sig2T,double sig2To,double sig2Uo,
              VectorXd startroot_X, VectorXd startroot_Y)
...
// Structure is 1) calculate scores based on loadings, 2) calculate loadings based on scores
  ...
  // Find lambda, if findrC(0) returns 12345 (=undefined) then divide by norm
  VectorXd lambdaX = (multirootC(startroot_X,Cxt,Cxto,Ctt(0,0),Ctto(0,0),Ctoto(0,0)));
  if(lambdaX(0) != 12345){
    MatrixXd WPY = findrC_matrix(lambdaX,Cxt,Cxto,Ctt(0,0),Ctto(0,0),Ctoto(0,0));
    PYo = WPY.col(1);
    Wl = WPY.col(0);
    PYo /= PYo.norm();
    Wl /= Wl.norm();
  ...
  // Find lambda, if findrC returns 12345 (=undefined) then divide by norm
  VectorXd lambdaY = (multirootC(startroot_Y,Cyu,Cyuo,Cuu(0,0),Cuuo(0,0),Cuouo(0,0)));
  if(lambdaY(0) != 12345){
    MatrixXd CPX = findrC_matrix(lambdaY,Cyu,Cyuo,Cuu(0,0),Cuuo(0,0),Cuouo(0,0));
    PXo = CPX.col(1);
    Cl = CPX.col(0);
    PXo /= PXo.norm();
    Cl /= Cl.norm();
  ...
  ret["lambda_2"] = 1;//lambda_2;
  ret["lambda_4"] = 2;//lambda_4;

  return ret;

}

List EMC(int maxit, List ret, MatrixXd Dat,VectorXd startroot_X,VectorXd startroot_Y)
...
int main() {
	int pq = 200;
	int N = 500;
	VectorXd W = VectorXd::Ones(pq);
	MatrixXd Dat = MatrixXd::Ones(N,2*pq);

	cout << loglC(W,W,W,W,1,Dat,1,1,1,1,1,1) << endl;
